...
/* To make the data 'static, we can intentionally leak the memory. */
...
// Unsafe and UB (Undefined Behavior)
let a = [10, 20, 30];
let index = 10; // Wrong 
...
/*  Interior Mutability 
Mutates data through an immutable reference by checking borrowing rules at 
runtime, using types like `Cell`, `RefCell`, etc.
